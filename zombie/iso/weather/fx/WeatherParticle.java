...
public abstract class WeatherParticle {
   ...
   public boolean isOnScreen(float var1, float var2) {
      int var3 = IsoCamera.frameState.OffscreenWidth;
      int var4 = IsoCamera.frameState.OffscreenHeight;
      float var5 = var1 + (float)this.bounds.getX();
      float var6 = var2 + (float)this.bounds.getY();
      float var7 = var5 + this.oWidth;
      float var8 = var6 + this.oHeight;
      if (!(var5 >= (float)var3) && !(var7 <= 0.0F)) {
         return !(var6 >= (float)var4) && !(var8 <= 0.0F);
      } else {
         return false;
      }
   }
